...
public class CactusShot : MonoBehaviour
{
    ...
    public Sprite cactus_shot0;
    public Sprite cactus_shot1;
    public Sprite cactus_shot2;
    public Sprite cactus_shot3;
    ...
    public void Update()
    {
        ...
        if (rc.CurrentRoom == enemyRoom)
        {
            ...
            if (currentSprite == cactus_shot0 && timer <= cd / 4)
            {
                currentSprite = cactus_shot1;
                spriteTimer = Time.time;
                this.GetComponent<SpriteRenderer>().sprite = currentSprite;
            }
            else if (currentSprite == cactus_shot3)
            {
                time = Time.time - spriteTimer;
                if (time >= 0.5f && spriteTimer != 0)
                {
                    spriteTimer = Time.time;
                    currentSprite = cactus_shot2;
                    this.GetComponent<SpriteRenderer>().sprite = currentSprite;
                }
            }
            else if (currentSprite == cactus_shot1)
            {
                time = Time.time - spriteTimer;
                if (time >= 0.5f && spriteTimer != 0)
                {
