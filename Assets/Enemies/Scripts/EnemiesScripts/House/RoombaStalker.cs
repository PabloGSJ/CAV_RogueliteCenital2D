...
public class RoombaStalker : MonoBehaviour
{
    ...
    public Sprite roomba_follow0;
    public Sprite roomba_follow1;
    public Sprite roomba_follow2;
    public Sprite currentSprite;

    private void Awake()
    {
        spriteTimer = 0;
        currentSprite = roomba_follow0;
    ...
    private void FixedUpdate()
    {
        ...
        if (rc.CurrentRoom == enemyRoom && player != null)
        {
            ...
            if (currentSprite == roomba_follow0)
            {
                time = Time.time - spriteTimer;
                if (time >= 0.5f)
                {
                    currentSprite = roomba_follow1;
                    spriteTimer = Time.time;
                    this.GetComponent<SpriteRenderer>().sprite = currentSprite;
                }
            }
            else if (currentSprite == roomba_follow1)
            {
                time = Time.time - spriteTimer;
                if (time >= 1f && spriteTimer != 0)
                {
                    currentSprite = roomba_follow2;
                    this.GetComponent<SpriteRenderer>().sprite = currentSprite;
                }
            }
