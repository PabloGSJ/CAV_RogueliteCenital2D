...
public class CarrotShot : MonoBehaviour
{
    ...
    public Sprite carrot_shot0;
    public Sprite carrot_shot1;
    public Sprite carrot_shot2;
    public Sprite carrot_shot3;
    ...
    public void Update()
    {
        ...
        if (rc.CurrentRoom == enemyRoom)
        {
            ...
            if (currentSprite == carrot_shot0 && timer <= cd / 4)
            {
                currentSprite = carrot_shot1;
                spriteTimer = Time.time;
                this.GetComponent<SpriteRenderer>().sprite = currentSprite;
            }
            else if (currentSprite == carrot_shot3)
            {
                time = Time.time - spriteTimer;
                if (time >= 0.5f && spriteTimer != 0)
                {
                    spriteTimer = Time.time;
                    currentSprite = carrot_shot2;
                    this.GetComponent<SpriteRenderer>().sprite = currentSprite;
                }
            }
            else if (currentSprite == carrot_shot1)
            {
                time = Time.time - spriteTimer;
                if (time >= 0.5f && spriteTimer != 0)
                {
