...
public class TomatoStalker : MonoBehaviour
{
    ...
    public Sprite tomato_follow0;
    public Sprite tomato_follow1;
    public Sprite tomato_follow2;
    public Sprite currentSprite;

    private void Awake()
    {
        spriteTimer = 0;
        currentSprite = tomato_follow0;
    ...
    private void FixedUpdate()
    {
        ...
        if (rc.CurrentRoom == enemyRoom && player != null)
        {
            ...
            if (currentSprite == tomato_follow0)
            {
                time = Time.time - spriteTimer;
                if (time >= 0.5f)
                {
                    currentSprite = tomato_follow1;
                    spriteTimer = Time.time;
                    this.GetComponent<SpriteRenderer>().sprite = currentSprite;
                }
            }
            else if (currentSprite == tomato_follow1)
            {
                time = Time.time - spriteTimer;
                if (time >= 1f && spriteTimer != 0)
                {
                    currentSprite = tomato_follow2;
                    this.GetComponent<SpriteRenderer>().sprite = currentSprite;
                }
            }
